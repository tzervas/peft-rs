        ...
        if n_butterfly_factor > 0 {
            ...
            if in_features < block_size * divisor {
                return Err(PeftError::InvalidConfig(format!(
                    ...
                    in_features, n_butterfly_factor, block_size * divisor
                )));
            }
...
impl Adapter for BoftLayer {
    ...
    fn num_parameters(&self) -> usize {
        ...
        let r_params = (self.n_butterfly_factor + 1) * self.block_num * self.block_size * self.block_size;
...
impl Trainable for BoftLayer {
    fn register_parameters(&self, var_map: &mut VarMap, prefix: &str) -> Result<()> {
        ...
        var_map.data().lock().unwrap().insert(
            boft_r_name,
            Var::from_tensor(&self.boft_r)?,
        );
        var_map.data().lock().unwrap().insert(
            boft_s_name,
            Var::from_tensor(&self.boft_s)?,
        );
