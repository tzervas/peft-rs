...
impl<A: Mergeable> PeftModel<A> {
    /// Merge each module's **active** adapter into the matching base weight.
    ///
    /// Why: callers that want a single state-dict for deployment need
    /// `W' = W + ΔW` per targeted module without keeping adapter tensors around.
    /// Key lookup accepts either bare `module_name` or `module_name.weight` so
    /// HF-style and bare maps both work. Modules without an active adapter are
    /// left unchanged; missing keys for modules that *do* have an active adapter
    /// fail loudly so silent incomplete merges cannot ship.
    ///
    /// # Errors
    /// Returns [`PeftError::InvalidConfig`] when a module with an active adapter
    /// has no matching key in `base_weights`, or propagates adapter merge errors.
    pub fn merge_weights(
        ...
        for (module_name, adapters) in &self.module_adapters {
            let Some(entry) = adapters.values().find(|e| e.active) else {
                continue;
            };
    ...
    /// Inverse of [`Self::merge_weights`]: peel active adapters off a merged map.
    ///
    /// Why: round-trip checks and "unmerge for further training" need the same
    /// key-resolution rules as merge so callers never have to special-case maps.
    ///
    /// # Errors
    /// Same missing-key and adapter-unmerge failure modes as [`Self::merge_weights`].
