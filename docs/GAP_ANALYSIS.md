...
| Inference helpers (`src/inference.rs`) | **partial** (switcher/mode/metrics/`merge_active`/`save_merged_model`; no eval harness) |
...
6. Inference: evaluation harness (state-dict merge+export helpers exist; no dataset loop)
