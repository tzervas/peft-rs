...
Honest implementation status for **peft-rs 1.3.x**. Prefer [README.md](../README.md)
...
| **AdaLoRA** | **partial** | SVD params + **top-k** rank mask + cubic schedule; dropout when unfrozen |
| **IA³ / LoHa / LoKr / OFT / BOFT / VeRA** | **partial** | Layer math; no HF key suite; BOFT `boft_dropout` fail-closed at 0 |
...
| **Training utilities** | **done** (minimal) | `train_step_mse` / `train_step_with_loss` + `max_grad_norm` clip — not full PeftTrainer |
...
- Honesty hygiene: drop orphan `inference.rs`; stop `--all-features` in release
  scripts; AdaLoRA dropout applied; BOFT dropout / HF bias fail-closed;
  `max_grad_norm` clips trainable grads; `freeze()` is a flag (skips dropout,
  does not detach Vars)
